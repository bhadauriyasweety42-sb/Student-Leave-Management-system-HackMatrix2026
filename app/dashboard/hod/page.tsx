...
export default function HODDashboard() {
  ...
  function mapHODRequest(request: any) {
    return {
      ...request,
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        const leavesRes: any = await xanoFetch('/hod/pending', { method: 'GET' }, 'leave')
        const normalizedLeaves = normalizeLeavesResponse(leavesRes)
        const mapped = normalizedLeaves.map(mapHODRequest)
        setRequests(mapped)
      } catch (err) {
        console.error(err)
        clearAuthToken()
  ...
  const handleApproval = async () => {
    ...
    try {
      const endpoint = `/hod/${action}/${selectedRequest.id}`
      await xanoFetch(endpoint, {
        method: 'POST',
        body: JSON.stringify({ hod_remark: comment }),
      }, 'leave')
