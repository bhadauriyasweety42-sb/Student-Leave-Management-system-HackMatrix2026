...
export default function CoordinatorSignUp() {
  ...
  const departmentIdMap: Record<string, string> = {
    ...
    IT: '10',
    'Engineering Mathematics & Computing (MAC)': '11',
  ...
  const branchIdMap: Record<string, string> = {
    ...
    IT: '16',
    'Engineering Mathematics & Computing (MAC)': '17',
  ...
  const branchOptions = departmentBranches[selectedDepartment] ?? []
  const sectionOptions = formData.branch
    ? [
        { id: '1', label: 'A' },
        { id: '2', label: 'B' },
        { id: '3', label: 'C' },
  ...
  const handleDepartmentSelect = (department: string) => {
    const nextBranchOptions = departmentBranches[department] ?? []
    const selectedBranch = nextBranchOptions.length === 1 ? nextBranchOptions[0] : ''

    setSelectedDepartment(department)
    setFormData(prev => ({
      ...prev,
      department_id: departmentIdMap[department] ?? '',
      branch: selectedBranch,
      branch_id: selectedBranch ? branchIdMap[selectedBranch] ?? '' : '',
      section_id: '',
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const selectedBranches = departmentBranches[selectedDepartment] || []
      if (!formData.branch) {
            ...
            <CardContent>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedDepartment} onValueChange={(value) => handleSelectChange('department', value)}>
                    <SelectTrigger id="department_id">
                      <SelectValue placeholder="SELECT YOUR DEPARTMENT" />
                    </SelectTrigger>
                    <SelectContent>
                      {Object.keys(departmentBranches).map(department => (
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger id="branch" disabled={!selectedDepartment || branchOptions.length === 0}>
                      <SelectValue placeholder={selectedDepartment ? 'SELECT YOUR BRANCH' : 'SELECT DEPARTMENT FIRST'} />
                    </SelectTrigger>
                    <SelectContent>
